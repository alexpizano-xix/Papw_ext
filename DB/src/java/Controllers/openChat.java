...
public class openChat extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<cart> carts = cart.getAllCart2();
        List<cart> cartsUser = cart.getAllCart(id2);
        List<User> users = User.searchUser2(username, pass2);
        List<product> products = product.getAllProduct();
        List<User> users2 = User.searchAllUsers();
        List<messages> message = messages.getAllMessages();
        
        request.setAttribute(RequestUtils.KEY_ALL_CART, carts);
        request.setAttribute(RequestUtils.KEY_CART, cartsUser);
        request.setAttribute(RequestUtils.KEY_USER, users);
        request.setAttribute(RequestUtils.KEY_ALL_PRODUCTS, products);
        request.setAttribute(RequestUtils.KEY_ALL_USERS, users2);
        request.setAttribute(RequestUtils.KEY_ALL_MESSAGE, message);
